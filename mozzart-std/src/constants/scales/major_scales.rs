...
lazy_static! {
    ...
    /// # Examples
    /// ```
    /// use mozzart_std::{constants::scales::major_scales::MAJOR_SCALES, C4, major_scale};
    ///
...
mod tests {
    use super::*;
    use crate::constants::{C4, CSHARP4, D4, DSHARP4, E4, F4, FSHARP4, G4, GSHARP4, A4, ASHARP4, B4};
    use crate::Pitch;
    ...
    fn test_major_scales_hashmap() {
        ...
        for root in roots.iter() {
            ...
            assert!(scale.is_some(), "MAJOR_SCALES should contain the root pitch");
            
            let scale = scale.unwrap();
            assert_eq!(scale.tonic(), *root, "Scale tonic should match the key");
            assert_eq!(scale.steps(), MAJOR_SCALE_STEPS, "Scale should follow major scale pattern");
        }
    }
    ...
    fn test_major_scales_hashmap_octave_boundaries() {
        ...
        assert!(lowest_scale.is_some(), "MAJOR_SCALES should contain the lowest pitch");
        
        // For highest supported pitch, ensure we have a scale
        let highest_scale = MAJOR_SCALES.get(&highest_supported);
        assert!(highest_scale.is_some(), "MAJOR_SCALES should contain the highest supported pitch");
    }
