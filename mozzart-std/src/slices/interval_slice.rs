...
pub struct IntervalSlice<'a>(&'a [Interval]);

impl<'a> IntervalSlice<'a> {
    /// Creates a new `Intervals` from a slice of intervals
    ///
    /// # Examples
    /// ```
    /// use mozzart_std::{Interval, IntervalSlice, MAJOR_THIRD, MINOR_THIRD};
    ///
    /// let major_triad = [MAJOR_THIRD, MINOR_THIRD];  // Major third, minor third
    /// let intervals = IntervalSlice::new(&major_triad);
    /// ```
    #[inline(always)]
    ...
    /// # Examples
    /// ```
    /// use mozzart_std::{Interval, IntervalSlice, Pitch, C4, E4, G4, MAJOR_THIRD, MINOR_THIRD};
    ///
    /// let c4 = C4;  // Middle C
    /// let major_triad = [MAJOR_THIRD, MINOR_THIRD];  // Major third, minor third
...
impl AsRef<[Interval]> for IntervalSlice<'_> {
...
impl<'a> From<&'a [Interval]> for IntervalSlice<'a> {
    ...
    /// # Examples
    /// ```
    /// use mozzart_std::{IntervalSlice, MAJOR_THIRD, MINOR_THIRD};
    ///
    /// let major_triad = [MAJOR_THIRD, MINOR_THIRD];  // Major third, minor third
    /// let intervals: IntervalSlice = (&major_triad[..]).into();
...
/// ```
impl fmt::Debug for IntervalSlice<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let intervals = NamedSlice::new("Intervals", self.0);
        write!(f, "{intervals:?}")
    }
}
...
mod tests {
    use crate::{
        Interval, IntervalSlice, C4, C5, E4, G4, MAJOR_THIRD, MINOR_THIRD, PERFECT_FOURTH,
    };
    ...
    fn test_intervals_compound_triad() {
        ...
        let pitches = IntervalSlice::new(&intervals).into_pitches(c4);
    ...
    fn test_intervals_as_ref() {
        let intervals = [MAJOR_THIRD, MINOR_THIRD]; // Major third, minor third
        let interval_slice = IntervalSlice::new(&intervals);
    ...
    fn test_intervals_from() {
        let intervals = [MAJOR_THIRD, MINOR_THIRD]; // Major third, minor third
        let interval_slice = IntervalSlice::from(&intervals[..]);
    ...
    fn test_intervals_into() {
        let intervals = [MAJOR_THIRD, MINOR_THIRD]; // Major third, minor third
        let interval_slice: IntervalSlice = (&intervals[..]).into();
        assert_eq!(interval_slice.as_ref(), &intervals[..]);
    }
    ...
    fn test_intervals_debug() {
        let intervals = [MAJOR_THIRD, MINOR_THIRD]; // Major third, minor third
        let formatted = format!("{:?}", IntervalSlice::new(&intervals));
